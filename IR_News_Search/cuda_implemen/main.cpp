...
{	
	...
	std::string query_publication{"buzzfeed ney york news business"};
	...
	for(int i = 0; i < zonal_structures.size();i++)
	   zonal_structures[i].Basic_Deallocation();

	if(cluster_yes == true)
	{
		for(int i=0; i < 3; i++)
		{
			zonal_structures[i].k_Deallocation_for_clustering();
			zonal_structures[i].Common_Deallocation_for_clustering();
		}
	}
